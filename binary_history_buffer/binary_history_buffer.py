...
class binary_history_buffer():
    ...
    def __len__(self) -> int:
        ...
        """
        return int(self.updates) if not self.limit else int(minimum(self.updates, self.limit))
    ...
    def __getitem__(self, index: int | integer | slice) -> bool | Self:
        ...
        limit: int = int(minimum(self.updates, self.limit) if self.limit else self.updates)
        ...
        if index >= limit or index < -limit:
            raise IndexError(f'Index {index} is out of range for history of length {limit}')
        if index < 0:
    ...
    def history_totals(self, start: int | integer, length: int | integer = 0) -> tuple[uint64, uint64, float64]:
