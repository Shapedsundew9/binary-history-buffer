...
from numpy import int64, uint64, uint32, uint8, int8, zeros, signedinteger, float64, arange, power, array, cumsum, where
...
class binary_history_buffer_log2_table():
    ...
    def __init__(self, size: int = 1, length: uint8 | int = uint8(6)) -> None:
        """Create a binary history buffer.

        Args:
            size (int, optional): Number of buffers to maintain in the table. Defaults to 1.
            length (int, optional): Log2(length of the buffer) - 6. Defaults to 6 (4096 bits)
        """
        self._size: int = size
        ...
        # bit_ranges: The range of history bits in each store (inclusive)
...
class binary_history_buffer_log2():
    ...
    def __init__(self, bhbl2t: binary_history_buffer_log2_table | None = None, entry: int = 0, length: uint8 | int = 6) -> None:
        """Create a binary history buffer.
        ...
        """
        self._bhbl2t: binary_history_buffer_log2_table = bhbl2t if bhbl2t is not None else binary_history_buffer_log2_table(length=length)
