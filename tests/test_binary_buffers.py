...
TEST_HF_RND_PATTERNS: list[LiteralString] = [
    "".join(
        [
            ("0" * i, "1" * i)[n & 1]
            for n, i in enumerate((array(abs(normal(size=2048) * 4), dtype=int64) + 1))
        ]
    )
    for _ in range(32)
]
TEST_LF_RND_PATTERNS: list[LiteralString] = [
    "".join(
        [
            ("0" * i, "1" * i)[n & 1]
            for n, i in enumerate((array(abs(normal(size=512) * 32), dtype=int64) + 1))
        ]
    )
    for _ in range(32)
]
...
def test_bhb_as_int(pattern: str, start: int, length: int, exception: bool) -> None:
    ...
    if exception:
        ...
        if not length:
            ...
            result = int(
                "1" + pattern[::-1][:15][start : max(start + length, start)][::-1], 2
            )
...
def test_bhb_getitem_index(
    limit: int, pattern: str, index: int, exception: bool
) -> None:
...
@pytest.mark.parametrize(
    "limit, pattern, start, stop, exception", TEST_GET_SLICE_PATTERNS
)
def test_bhb_getitem_slice(
    limit: int, pattern: str, start: int, stop: int, exception: bool
) -> None:
...
def test_bhb_rnd_patterns(pattern) -> None:
    ...
    ratio: float64 = (
        hits / updates if updates else float64(NAN)
    ...
    if _LOG_DEBUG:
        ...
        _logger.debug(
            f"start {start}, length {length}, # hits {hits}, # bits {updates}, ratio {ratio}"
        )
    ...
    if not length:
        ...
        assert bhb64.history(start, length).totals() == bhb64.history_totals(
            start, length
        )
...
def test_plot():
    ...
    for n, bit in enumerate(TEST_HF_RND_PATTERNS[0]):
        ...
        for i, (l2, bh) in enumerate(
            zip(
                zip(*bhbl264.histories()),
                (bhb64.history(2**n - 64) for n in range(7, 15)),
            )
        ):
    ...
    for i, c in enumerate(
        ...
        plt.plot(
            ratio_histories[0, i, :256] - ratio_histories[1, i, :256],
            linestyle="solid",
            color=c,
            linewidth=1,
        )
